...
train=pd.read_excel(share_url+'poppulation.xlsx')#인구, 물 양을 데이터로 읽어옴
f=open(share_url+'rez_시도.txt',"r")
f1=open(share_url+'wtater_ton_final.csv',"r")
...
for i in water_ton:
    ...
    if temp_ton[0]=='세종특별자치시':#세종시는 군, 구가 없기 때문에 별도로 처리
        ...
        only_water.append(temp_ton[2].replace('\n',''))#지역의 물 양에서 공백을 없앰
print(rezon_fi_check)
#나중에 시도 삭제하는데 사용
rezon_all=f.readline().split()
for i in range(len(rezon_all)):
    rezon_all[i]=rezon_all[i]+"  "
#인구수가 0명인 지역 삭제
...
sejong=train.loc[(train['지역'].isin(['세종특별자치시  ']))].index.tolist()[0]#세종시는 따로 처리
print(sejong)
#필요 없는 데이터 삭제
...
delete_list.append(sejong)

train=train.drop(delete_list)
train = train.sort_values(['지역'],ascending=True)
print(len(only_water))
